# -*- coding: utf-8 -*-
# @Time    : 2022/1/13 20:41
# @Author  : Yu Pengfei
# @Email   : 1837580905@qq.com
# @File    : predict_assess.py
# @Software: PyCharm
import os
import pandas as pd
...
results = model.predict(ds_test_encoded, verbose=1)
y_pred = results[0].argmax(axis=1)
raw_text_test = pd.read_csv(test_path, keep_default_na=False)
y_test = raw_text_test["polarity_id"]
print('accuracy %s' % accuracy_score(y_pred, y_test))
polarity = raw_text_test[['polarity', 'polarity_id']].drop_duplicates().sort_values('polarity_id').reset_index(drop=True)
print(classification_report(y_test, y_pred, target_names=polarity['polarity'].values, digits=4))
